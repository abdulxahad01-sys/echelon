...
const TrialCountdown = () => {
  ...
  return (
    <div
      className={`mb-6 p-4 sm:p-5 rounded-xl border transition-all duration-300 ${
        isExpiringSoon
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-start sm:items-center gap-3">
          ...
          >
            {isExpiringSoon ? (
              ...
              <Clock className="w-5 h-5 sm:w-6 sm:h-6 text-[#A17E3C]" />
            )}
          </div>

          <div className="space-y-1">
            <h3
              className={`font-semibold font-['IvyMode'] text-base sm:text-lg ${
                isExpiringSoon ? "text-orange-400" : "text-[#A17E3C]"
              }`}
            >
              {isExpiringSoon ? "Trial Expiring Soon!" : "Free Trial Active"}
            </h3>
            <p className="text-white/80 text-sm sm:text-base font-['Jost'] leading-snug">
              {isExpiringSoon
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 w-full sm:w-auto">
          <button
            ...
            className={`w-full sm:w-auto px-4 py-2 rounded-lg text-sm sm:text-base font-['Jost'] flex justify-center items-center gap-2 transition-colors ${
              isExpiringSoon
          ...
          <button
            onClick={() => navigate("/membership")}
            className="w-full sm:w-auto px-4 py-2 rounded-lg text-sm sm:text-base font-['Jost'] border border-[#A17E3C] text-[#A17E3C] hover:bg-[#A17E3C]/10 transition-colors"
          >
            Manage
          </button>
        </div>
