...
function ReportsPage() {
  const { reports, loadReports, resolveReport, deletePost, isLoading } = useAdmin();
  ...
  const handleResolveReport = (action, notes) => {
    if (selectedReport) {
      resolveReport(selectedReport.id, action, notes);
    }
  };
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 rounded-3xl border border-[#A17E3C]/40">
      <h1 className='text-white text-xl sm:text-2xl font-["Jost"] mb-4 sm:mb-6'>Content Reports</h1>
      ...
      <div className="space-y-3 sm:space-y-4">
        {reports.length === 0 ? (
          ...
          <div key={report.id} className="w-full bg-black/50 rounded-xl border border-[#A17E3C] overflow-hidden">
            {/* Mobile Layout */}
            <div className="block xl:hidden p-4 space-y-4">
              <div className="flex items-center gap-3">
                <img
                  src={report.reporter?.avatar_url || '/placeholder-avatar.png'}
                ...
                <div className="min-w-0 flex-1">
                  <h1 className='text-white text-sm sm:text-base font-medium font-["Jost"] truncate'>{report.reporter?.full_name || 'Unknown'}</h1>
                  <p className='text-white text-xs sm:text-sm font-["Jost"] truncate'>@{report.reporter?.handle || report.reporter?.email}</p>
                  <span className={`text-xs font-['Jost'] ${
                    report.status === 'open' ? 'text-yellow-400' :
                    report.status === 'actioned' ? 'text-green-400' : 'text-gray-400'
                  }`}>
                    {report.status}
                  </span>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs sm:text-sm">
                <div>
                  <h1 className="text-[#E6D2A2] font-['Jost']">Reported: {report.reported_user ? 'User' : 'Post'}</h1>
                  <h1 className="text-white font-['Jost'] truncate">
                    {report.reported_user?.full_name || report.reported_post?.content?.substring(0, 30) + '...' || 'Unknown'}
                  </h1>
                </div>
                <div>
                  <h1 className="text-[#E6D2A2] font-['Jost']">Reason</h1>
                  <h1 className="text-white font-['Jost'] truncate">{report.reason}</h1>
                </div>
                <div className="sm:col-span-2">
                  <h1 className="text-[#E6D2A2] font-['Jost']">Reported on</h1>
                  <h1 className="text-white font-['Jost']">{moment(report.created_at).calendar()}</h1>
                </div>
              </div>
            ...
            <div className="hidden xl:block">
              <div className="flex items-center">
                <div className="flex-1 py-4 px-5">
                  <div className="flex items-center gap-4">
                    <img
                      src={report.reporter?.avatar_url || '/placeholder-avatar.png'}
                      alt={report.reporter?.full_name || 'Reporter'}
                      className="size-12 rounded-full bg-amber-500 object-cover flex-shrink-0"
                    />
                    <div className="min-w-0">
                      <h1 className='text-white text-base font-medium font-["Jost"] truncate'>{report.reporter?.full_name || 'Unknown'}</h1>
                      <p className='text-white text-sm font-["Jost"] truncate'>@{report.reporter?.handle || report.reporter?.email}</p>
                      <span className={`text-xs font-['Jost'] ${
                        report.status === 'open' ? 'text-yellow-400' :
                        report.status === 'actioned' ? 'text-green-400' : 'text-gray-400'
                      }`}>
                ...
                <div className="flex-1 py-4 px-3">
                  <h1 className="text-[#E6D2A2] font-['Jost'] text-sm">Reported Content</h1>
                  <h1 className="text-white font-['Jost'] truncate">
                    {report.reported_user?.full_name || report.reported_post?.content?.substring(0, 30) + '...' || 'Unknown'}
                  </h1>
                </div>
                <div className="flex-1 py-4 px-3">
                  <h1 className="text-[#E6D2A2] font-['Jost'] text-sm">Reason</h1>
                  <h1 className="text-white font-['Jost'] truncate">{report.reason}</h1>
                </div>
                <div className="flex-1 py-4 px-3">
                  <h1 className="text-[#E6D2A2] font-['Jost'] text-sm">Reported on</h1>
                  <h1 className="text-white font-['Jost']">{moment(report.created_at).calendar()}</h1>
                </div>
                <div className="flex-1 py-4 px-3">
